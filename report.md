...
(tasks 4 to 6), `figures.py`. The load-bearing parts are reproduced in the appendix.

## The dataset

The EMIP dataset [1] holds eye-tracking recordings of programmers reading two Java programs
(`vehicle` and `rectangle`) and answering a comprehension question about each. The 33 files used
here are 250 Hz SMI RED250 recordings of 33 000 to 84 000 samples and 45 columns, each behind a
metadata header of 32 to 45 lines. The rows are of two types: `SMP` measurements and `MSG`
...
that look like measurements, never as `NaN`.
...
samples is 71 ms; three files also contain a pause of up to 275 s before the first stimulus.
...
*Figure 1. Hole durations, all 33 files. 74 % of holes are under 20 ms but cost 5 % of the
...
| `blink`   | 50 to 400 ms | blink; [2, 3] report roughly 100 to 400 ms, and 50 ms is a cautious floor     |
...
little across a short blink, so a straight line between the endpoints is the conventional
approximation in pupil and gaze preprocessing [2, 4]. The cut-off was raised from 100 to
...
say little about the interior, and [4] set their own limit at 250 ms. The exclusion decision
...
behaviour: blink rate indexes visual attention demand [3], and long losses usually mean the
...
Peak saccade velocity grows with amplitude along the main sequence and saturates well below
1000 °/s [5], so a velocity above that limit is an artefact. Velocity is computed as
...
map with no tails to speak of, and Tukey's 1.5 × IQR fence [6] lands inside the stimulus.
...
Standardization is used instead of Min-Max because the extreme values were capped, not
removed, in task 3. The tails remain heavy, and Min-Max would compress the bulk of each column
into a narrow band around the midpoint. Only the 27 continuous columns are scaled:
...
standard deviation into the training features, a form of leakage [7]. The output is
...
`GroupShuffleSplit` on participant [8] with `test_size` 0.2 and seed 42, realised as 24/7
...
performance, because memorising is always available, and that is overfitting.
...
PCA [9] fitted on the scaled training features only, for the same leakage reason as the
scaler.
...
1. Bednarik, R., Busjahn, T., Gibaldi, A., Ahadi, A., Bielikova, M., Crosby, M., Essig, K.,
   Fagerholm, F., Jbara, A., Lister, R., Orlov, P., Paterson, J., Sharif, B., Sirkiä, T.,
   Stelovsky, J., Tvarozek, J., Vrzakova, H., & van der Linde, I. (2020). EMIP: The eye
   movements in programming dataset. *Science of Computer Programming*, 198, 102520.
   doi:10.1016/j.scico.2020.102520
2. Holmqvist, K., Nyström, M., Andersson, R., Dewhurst, R., Jarodzka, H., & van de Weijer, J.
   (2011). *Eye Tracking: A Comprehensive Guide to Methods and Measures*. Oxford University
   Press.
3. Stern, J. A., Walrath, L. C., & Goldstein, R. (1984). The endogenous eyeblink.
   *Psychophysiology*, 21(1), 22–33. doi:10.1111/j.1469-8986.1984.tb02312.x
4. Kret, M. E., & Sjak-Shie, E. E. (2019). Preprocessing pupil size data: Guidelines and code.
   *Behavior Research Methods*, 51(3), 1336–1342. doi:10.3758/s13428-018-1075-y
5. Bahill, A. T., Clark, M. R., & Stark, L. (1975). The main sequence, a tool for studying
   human eye movements. *Mathematical Biosciences*, 24(3–4), 191–204.
   doi:10.1016/0025-5564(75)90075-9
6. Tukey, J. W. (1977). *Exploratory Data Analysis*. Addison-Wesley.
7. Kaufman, S., Rosset, S., & Perlich, C. (2011). Leakage in data mining: Formulation,
   detection, and avoidance. In *Proceedings of the 17th ACM SIGKDD International Conference
   on Knowledge Discovery and Data Mining* (pp. 556–563). doi:10.1145/2020408.2020496
8. Pedregosa, F., et al. (2011). Scikit-learn: Machine learning in Python. *Journal of Machine
   Learning Research*, 12, 2825–2830.
9. Jolliffe, I. T., & Cadima, J. (2016). Principal component analysis: a review and recent
...
Interpolation against `Time` within a stimulus, with long holes restored to `NaN`
(task 2, `cleaning.py`; `group` numbers the runs and restarts at every stimulus change):

```python
run_len = missing.groupby(group).transform("size")
hole_ms = run_len * 1000 / rate
long_ = missing & (hole_ms > SHORT_MS)                   # SHORT_MS = 150

smp.loc[missing, signal] = np.nan                        # zeros would pull the fill to (0,0)
...
smp.loc[long_, signal] = np.nan                          # long holes stay NaN and are deleted
smp["quality"] = np.where(~missing, "ok",
                 np.where(smp["L POR X [px]"].notna(), "interpolated", "gap"))
```

Grouped split, then scaler and PCA fit on the training set only (tasks 4 to 6, `transform.py`):

```python
gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
tr, te = next(gss.split(X, y, groups))                   # groups = participant id
Xtr, Xte = X.iloc[tr].copy(), X.iloc[te].copy()

sc = StandardScaler().fit(Xtr[continuous])               # train only
Xtr[continuous] = sc.transform(Xtr[continuous])
Xte[continuous] = sc.transform(Xte[continuous])
pca = PCA().fit(Xtr[continuous])                         # train only
```
